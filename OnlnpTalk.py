...
class SimpleEncoder:  
    ...
    # None 방지 필터
        ...
        if len(self.h_list) == 0:
            return np.zeros((0, self.b_h.shape[0]))
...
# 모델 로드
model = joblib.load("Onlnp-LiteSeq.joblib")
...
# 입력 텍스트를 인덱스로 변환하는 함수
def tokenize_input(input_text, word2idx):
    tokens = input_text.split()  # 공백 기준으로 토큰화
    return [word2idx.get(word, word2idx.get("<unk>")) for word in tokens]  # 존재하지 않는 단어는 <unk>로 처리

# 대화형 인터페이스
def chat():
    print("대화형 챗봇에 오신 걸 환영합니다! 종료하려면 'exit'을 입력하세요.")
    while True:
        user_input = input("You: ")
        if user_input.lower() == "exit":
            print("대화가 종료되었습니다. 안녕!")
            break
        # 입력을 토큰화하여 모델에 전달
        x_seq = tokenize_input("<start> " + user_input, word2idx)
        response = model.predict(x_seq, word2idx, idx2word)
        print("Bot:", response)
